    ...
    <div className="min-h-screen bg-[#f3f7fb] text-[#101828]">
      <div className="mx-auto flex min-h-screen max-w-[1600px] bg-white shadow-2xl">
        <aside className="hidden w-[238px] shrink-0 border-r border-[#dfe5ee] bg-white lg:block">
          ...
          <div className="flex h-[calc(100vh-78px)] flex-col p-3">
            ...
            <nav className="space-y-1 text-[14px] flex-1 overflow-y-auto">
            ...
            <div className="mt-4 rounded-lg border border-slate-200 bg-slate-50 p-3 text-[10px] leading-4 text-slate-500">
              <p className="font-bold text-[#007e6f] text-[11px]">VUELA Interactive Sandbox</p>
              <p className="mt-1">Guided demo of our live CRM. Layouts or colors may vary with updates, but the core tools and workflows work the same.</p>
            </div>
          </div>
        </aside>
...
function GlobalModal({ type, callLog = [], onClose, onGoToMessages }) {
  ...
  const content = {
    ai: {
      title: "Ask AI",
      subtitle: "Demo assistant for quick CRM help.",
      body: (
        <div className="space-y-3">
          <div className="rounded-xl bg-slate-50 p-4 text-sm text-slate-600">Try asking: “Summarize today’s leads” or “Create a follow up task for Sarah.”</div>
          <div className="rounded-xl border border-slate-200 p-3 text-sm">What leads need attention today?</div>
          <div className="rounded-xl bg-[#f0faf8] p-4 text-sm text-slate-700">You have 3 warm leads, 2 unread conversations, and 1 invoice that needs follow up.</div>
          <button className="w-full rounded-xl bg-[#007e6f] px-4 py-3 font-bold text-white">Generate Summary</button>
        </div>
      ),
    },
    notifications: {
      title: "Notifications",
      subtitle: "Recent activity in this demo account.",
      body: (
        <div className="space-y-3">
          {["New message from Maria Lopez", "Avery Mitchell booked an appointment", "Invoice INV-1048 was paid", "Review request sent to Sofia Rivera"].map((item) => (
            <div key={item} className="flex items-center gap-3 rounded-xl border border-slate-200 p-3 text-sm"><Bell className="h-4 w-4 text-[#007e6f]" />{item}</div>
          ))}
        </div>
      ),
    },
    quickAdd: {
      title: "Quick Add",
      subtitle: "Create sample CRM items in demo mode.",
      body: (
        <div className="grid gap-3 sm:grid-cols-2">
          {["New Contact", "New Opportunity", "New Task", "Appointment", "Invoice", "Review Request"].map((item) => (
            <button key={item} onClick={() => alert(`${item} created in demo mode`)} className="rounded-xl border border-slate-200 p-4 text-left font-semibold transition hover:border-[#007e6f] hover:bg-[#f0faf8]">{item}</button>
          ))}
        </div>
      ),
    },
    search: {
      title: "Search Demo",
      subtitle: "Search across contacts, conversations, payments, and workflows.",
      body: (
        <div className="space-y-4">
          <div className="flex items-center gap-2 rounded-xl border border-slate-300 p-3"><Search className="h-4 w-4 text-slate-400" /><input autoFocus className="flex-1 bg-transparent outline-none" placeholder="Search Sarah, invoice, workflow..." /></div>
          <div className="space-y-2">
            {["Sarah Johnson · Contact", "New Lead Follow Up · Workflow", "INV-1003 · Invoice", "Marketing Pipeline · Opportunities"].map((item) => (
              <button key={item} className="block w-full rounded-xl border border-slate-200 p-3 text-left text-sm transition hover:bg-[#f0faf8]">{item}</button>
            ))}
          </div>
        </div>
      ),
    ...
    createOpportunity: {
      ...
      subtitle: "Add a new lead into the sample pipeline.",
      body: (
        <div className="space-y-3">
          <input className="w-full rounded-xl border border-slate-300 p-3 outline-none" placeholder="Contact name" />
          <input className="w-full rounded-xl border border-slate-300 p-3 outline-none" placeholder="Opportunity value" />
          <select className="w-full rounded-xl border border-slate-300 p-3 outline-none"><option>New Lead</option><option>Hot Lead</option><option>New Booking</option><option>Visit Attended</option><option>Job Won</option></select>
          <button onClick={() => alert('Opportunity created in demo mode')} className="w-full rounded-xl bg-[#007e6f] px-4 py-3 font-bold text-white">Create Demo Opportunity</button>
        </div>
      ),
    },
    createAppointment: {
    ...
    missedCall: {
      ...
      body: (
        <div className="space-y-3">
          {(callLog.length ? callLog : [{ name: 'Maria Lopez', phone: '+1 (503) 555-0148', message: 'Hi, I just called. Can someone text me back?', time: 'Just now' }]).slice(0, 3).map((call, index) => (
            <div key={index} className="rounded-xl border border-slate-200 p-4">
              <p className="font-bold">{call.name}</p>
              <p className="text-sm text-slate-500">{call.phone}</p>
              <p className="mt-3 rounded-lg bg-[#f0faf8] p-3 text-sm text-slate-700">{call.message}</p>
              <p className="mt-2 text-xs text-slate-400">{call.time}</p>
            </div>
          ))}
          <button onClick={onGoToMessages} className="w-full rounded-xl bg-[#007e6f] px-4 py-3 font-bold text-white">Send Missed Call Text Back</button>
        </div>
      ),
    },
  }[type];
    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-slate-950/35 p-4 backdrop-blur-sm">
      <div className="w-full max-w-xl overflow-hidden rounded-2xl bg-white shadow-2xl">
        <div className="flex items-start justify-between border-b border-slate-200 p-5">
          ...
          <button onClick={onClose} className="rounded-full bg-slate-100 px-3 py-1 text-sm font-bold text-slate-600">Close</button>
        </div>
...
function OpportunityDrawer({ opportunity, stage, onStageChange, onClose }) {
  ...
  return (
    <div className="fixed inset-y-0 right-0 z-50 w-full max-w-md border-l border-slate-200 bg-white shadow-2xl">
      <div className="flex items-start justify-between border-b border-slate-200 p-5">
        <div>
          <p className="text-xs font-bold uppercase tracking-wide text-[#007e6f]">Opportunity Detail</p>
          <h3 className="mt-1 text-2xl font-bold">{opportunity.name}</h3>
          <p className="text-sm text-slate-500">{opportunity.service} · ${opportunity.value}</p>
        </div>
      ...
      <div className="space-y-5 p-5">
        <div className="rounded-xl border border-slate-200 p-4">
          ...
          <p className="mt-2 text-sm text-slate-600">{opportunity.phone}</p>
          <p className="text-sm text-slate-600">{opportunity.email}</p>
        </div>
        <div>
          <p className="mb-2 text-sm font-bold">Move Stage</p>
          <div className="grid grid-cols-2 gap-2">
            {stages.map((s) => (
              <button key={s} onClick={() => onStageChange(s)} className={`rounded-xl border p-3 text-left text-sm font-semibold ${stage===s ? 'border-[#007e6f] bg-[#f0faf8] text-[#007e6f]' : 'border-slate-200 hover:bg-slate-50'}`}>{s}</button>
            ))}
          </div>
        </div>
        <div className="grid grid-cols-3 gap-2">
          <button onClick={() => alert('Note added in demo mode')} className="rounded-xl border border-slate-200 p-3 text-sm font-bold hover:bg-[#f0faf8]">Add Note</button>
          <button onClick={() => alert('Task created in demo mode')} className="rounded-xl border border-slate-200 p-3 text-sm font-bold hover:bg-[#f0faf8]">Add Task</button>
          <button onClick={() => alert('Appointment scheduled in demo mode')} className="rounded-xl border border-slate-200 p-3 text-sm font-bold hover:bg-[#f0faf8]">Appointment</button>
        </div>
